...
class Wireless(object):
    ...
    def getFragmentation(self):
        """returns fragmentation threshold 
           ...
           you'll get a string: 'off'.
            ...
            'off'
        """
        iwparam = Iwparam(self.ifname, flags.SIOCGIWFRAG)
    ...
    def getPowermanagement(self):
        """returns power management settings 
            ...
            'off'
        """
        iwparam = Iwparam(self.ifname, flags.SIOCGIWPOWER)
        if iwparam.errorflag:
            return (iwparam.errorflag, iwparam.error)
        return iwparam.getValue()
    ...
    def getRetrylimit(self):
        """returns limit retry/lifetime
            ...
            16
        """
        iwparam = Iwparam(self.ifname, flags.SIOCGIWRETRY)
        if iwparam.errorflag:
            return (iwparam.errorflag, iwparam.error)
        return iwparam.getValue()
    ...
    def getRTS(self):
        """returns rts threshold 
            ...
            'off'
        """
        iwparam = Iwparam(self.ifname, flags.SIOCGIWRTS)
        if iwparam.errorflag:
            return (iwparam.errorflag, iwparam.error)
        return iwparam.getValue()
    ...
    def getSensitivity(self):
        ...
        """
        iwparam = Iwparam(self.ifname, flags.SIOCGIWSENS)
        if iwparam.errorflag:
            return (iwparam.errorflag, iwparam.error)
        return iwparam.getValue()
...
class Iwparam(object):
    ...
    def getValue(self):
        ...
        if self.disabled:
            return 'off'
        if self.flags & flags.IW_RETRY_TYPE == 0:
    ...
    def getPMAttributes(self):
        ...
        # Modifiers
        if self.flags & flags.IW_POWER_MIN == 0:
            result = " min"
        if self.flags & flags.IW_POWER_MAX == 0:
            result = " max"
            
        # Type
        if self.flags & flags.IW_POWER_TIMEOUT == 0:
            result = " period:" 
        else:
            result = " timeout:"
        # Value with or without units
        # IW_POWER_RELATIVE - value is *not* in s/ms/us
        if self.flags & flags.IW_POWER_RELATIVE:
...
class Iwpoint(object):
    ...
    def getEncryptionKey(self):
        ...
        if self.flags & flags.IW_ENCODE_DISABLED != 0:
            return 'off'
        elif self.flags & flags.IW_ENCODE_NOKEY != 0:
...
class Iwscan(object):
    ...
    def scan(self, fullscan=True):
        ...
        if fullscan:
            self.setScan()
            if self.errorflag > flags.EPERM:
                errormsg = "setScan failure %s %s" % (str(self.errorflag),
                                                   str(self.error))
                raise RuntimeError(errormsg)
                return None
            elif self.errorflag < flags.EPERM:
    ...
    def _parse(self, data):
        ...
        while 1:
            ...
            # header, break
            if len(data) < flags.IW_EV_LCP_LEN:
                break;
...
class Iwscanresult(object):
    ...
    def addEvent(self, cmd, data):
        ...
        """
        if cmd <= flags.SIOCIWLAST:
            if cmd < flags.SIOCIWFIRST:
                return None
        elif cmd >= flags.IWEVFIRST:
            if cmd > flags.IWEVLAST:
                return None
        else:
            return None
            
        if cmd == flags.SIOCGIWESSID:
            self.essid = data[4:]
